...
class ManipConfig:
    ...
    max_ev: np.ndarray = np.array(
        [0.2, 0.2, 0.2, 0.4, 0.4,
         0.4])  # Maximum cartesian velocity of the end effector
    max_qd: np.ndarray = np.array(
        [0.3, 0.3, 0.3, 0.6, 0.6,
         1.0])  # Maximum velocity of each of the arm joints
...
def load_ros_params() -> ManipConfig:
    ...
    # Control parameters
    config.control_frequency = rospy.get_param("control_frequency",
                                               config.control_frequency)
    config.command_timeout = rospy.get_param("command_timeout",
                                             config.command_timeout)
    config.servo_gain = np.array(
        rospy.get_param("servo_gain", config.servo_gain.tolist()))
    config.max_setpoint_position_distance = rospy.get_param(
        "max_setpoint_position_distance",
        config.max_setpoint_position_distance)
    config.max_setpoint_orientation_distance = rospy.get_param(
        "max_setpoint_orientation_distance",
        config.max_setpoint_orientation_distance)
    config.trajectory_duration = rospy.get_param("trajectory_duration",
                                                 config.trajectory_duration)
    ...
    # Robot parameters
    config.robot_urdf_path = rospy.get_param("robot_urdf_path",
                                             config.robot_urdf_path)
    config.robot_joint_names = rospy.get_param("robot_joint_names",
                                               config.robot_joint_names)
    config.base_frame_id = rospy.get_param("base_frame_id",
                                           config.base_frame_id)
    config.ee_frame_id = rospy.get_param("ee_frame_id", config.ee_frame_id)
    config.twist_cmd_frame_id = rospy.get_param("twist_cmd_frame_id",
                                                config.twist_cmd_frame_id)
    ...
    config.gripper_cmd_topic = rospy.get_param("gripper_cmd_topic",
                                               config.gripper_cmd_topic)
    config.preset_request_topic = rospy.get_param("preset_request_topic",
                                                  config.preset_request_topic)
    config.robot_state_topic = rospy.get_param("robot_state_topic",
                                               config.robot_state_topic)
    config.robot_command_topic = rospy.get_param("robot_command_topic",
