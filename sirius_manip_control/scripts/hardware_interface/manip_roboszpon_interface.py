...
class ManipRoboszponInterface(ManipHardwareInterface):
    ...
    def _initialize_hardware_connection(self):
        ...
        self.joint_positions = {
            key: 0.0
            for key in self.feedback_topics.keys()
        }
        ...
        for joint, topic in self.command_topics.items():
            self.command_publishers[joint] = rospy.Publisher(topic,
                                                             SiriusJointState,
                                                             queue_size=10)
            info = info + f"\n    - {joint}: {topic}"
    ...
    def _send_hardware_command(self, joint_state: JointState):
        for index, name in enumerate(joint_state.name):
            ...
            if (not hasattr(self, "command_publishers")
